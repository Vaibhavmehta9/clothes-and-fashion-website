...
const UNIQUE_IMAGES = {
  ...
  saree: [
    'photo-1583391733981-8498408ee4b2',
    'photo-1617627143233-4df547d06a15',
...
function getUniqueImagesForCategory(type: keyof typeof UNIQUE_IMAGES, count: number = 4): string[] {
  const images = UNIQUE_IMAGES[type];
  if (!images) return [];
  ...
  for (let i = 0; i < count; i++) {
    ...
    selected.push(`${CLOUDINARY_BASE}/${images[idx]}?w=800`);
  }
  
  // Advance the counter so next products of the same type get different images
  imageCounters[type] = (startIndex + 2) % images.length;
  
  return selected;
}

function getImageType(category: string, name: string): keyof typeof UNIQUE_IMAGES {
  ...
  if (lowerName.includes('jacket') || lowerName.includes('coat') || lowerName.includes('blazer') || lowerName.includes('tracksuit')) {
    return 'jacket';
  }
...
const PRODUCT_IMAGES = {
  tshirt: getUniqueImagesForCategory('tshirt', 12),
  jeans: getUniqueImagesForCategory('jeans', 12),
  shoes: getUniqueImagesForCategory('shoes', 12),
  dress: getUniqueImagesForCategory('dress', 12),
  jacket: getUniqueImagesForCategory('jacket', 12),
  saree: getUniqueImagesForCategory('saree', 12),
  watch: getUniqueImagesForCategory('watch', 12),
  bag: getUniqueImagesForCategory('bag', 12),
  hoodie: getUniqueImagesForCategory('hoodie', 12),
  kurti: getUniqueImagesForCategory('kurti', 12),
};

const CATEGORY_DATA = [
  { name: 'Men', slug: 'men', icon: '👔', image: 'https://images.unsplash.com/photo-1490578474895-699cd4e2cf59?auto=format&fit=crop&w=800&q=80', description: 'Explore the latest in men\'s fashion' },
  { name: 'Women', slug: 'women', icon: '👗', image: 'https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=800&q=80', description: 'Trendy fashion for modern women' },
  { name: 'Kids', slug: 'kids', icon: '👶', image: 'https://images.unsplash.com/photo-1503944168849-8bf86875bbd8?auto=format&fit=crop&w=800&q=80', description: 'Fun and comfortable kids\' clothing' },
  { name: 'Footwear', slug: 'footwear', icon: '👟', image: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?auto=format&fit=crop&w=800&q=80', description: 'Shoes for every occasion' },
  { name: 'Accessories', slug: 'accessories', icon: '👜', image: 'https://images.unsplash.com/photo-1553062407-98eeb64c6a62?auto=format&fit=crop&w=800&q=80', description: 'Complete your look' },
  { name: 'Ethnic Wear', slug: 'ethnic-wear', icon: '🪔', image: 'https://images.unsplash.com/photo-1617627143233-4df547d06a15?auto=format&fit=crop&w=800&q=80', description: 'Celebrate Indian heritage' },
  { name: 'Sports Wear', slug: 'sports-wear', icon: '🏃', image: 'https://images.unsplash.com/photo-1517963879433-6ad2b056d712?auto=format&fit=crop&w=800&q=80', description: 'Performance activewear' },
  { name: 'Winter Collection', slug: 'winter-collection', icon: '❄️', image: 'https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=800&q=80', description: 'Stay warm in style' },
  { name: 'Watches', slug: 'watches', icon: '⌚', image: 'https://images.unsplash.com/photo-1524592094714-0f0654e20314?auto=format&fit=crop&w=800&q=80', description: 'Luxury & casual timepieces' },
  { name: 'Bags', slug: 'bags', icon: '🎒', image: 'https://images.unsplash.com/photo-1548036328-c9fa89d128fa?auto=format&fit=crop&w=800&q=80', description: 'Backpacks, totes & more' },
];
...
async function seed() {
  ...
  await Promise.all([
    ...
    Notification.deleteMany({}), Newsletter.deleteMany({}), Question.deleteMany({}),
  ]);
  ...
  const PRODUCTS_DATA = [
    ...
    // New items added to reach higher volume and variety:
    { name: 'Nike Sportswear Tech Fleece Tracksuit', brand: 'nike', category: 'sports-wear', vendor: 2,
      desc: 'Nike Tech Fleece wraps you in lightweight warmth. This premium tracksuit features modern tapering, a full-zip hoodie design, and zippered utility pockets.', short: 'Premium lightweight Tech Fleece tracksuit', images: PRODUCT_IMAGES.jacket,
      variants: SIZES.map((s, i) => ({ size: s, color: 'Black', colorCode: '#000000', price: 9999, mrp: 14999, stock: 12, images: PRODUCT_IMAGES.jacket, sku: generateSKU('NIK5', i + 1) })),
      tags: ['nike', 'tech fleece', 'tracksuit', 'sport', 'streetwear'], isFeatured: true, isNewArrival: true, isTrending: true, isBestSeller: false },
    { name: 'Zara Premium Linen Summer Shirt', brand: 'zara', category: 'men', vendor: 0,
      desc: 'Stay cool and breezy in this premium 100% linen shirt from Zara. Features a relaxed fit, camp collar, and light textures perfect for warm summer days.', short: 'Breezy 100% linen shirt for hot summer days', images: PRODUCT_IMAGES.tshirt,
      variants: SIZES.map((s, i) => ({ size: s, color: 'White', colorCode: '#FFFFFF', price: 2999, mrp: 4999, stock: 15, images: PRODUCT_IMAGES.tshirt, sku: generateSKU('ZAR4', i + 1) })),
      tags: ['zara', 'linen', 'shirt', 'summer', 'men'], isFeatured: false, isNewArrival: true, isTrending: false, isBestSeller: false },
    { name: 'H&M Oversized Denim Utility Jacket', brand: 'hm', category: 'winter-collection', vendor: 0,
      desc: 'A rugged, oversized denim jacket with classic metal buttons, chest flap pockets, and additional cargo-style utility pockets. Perfect for layering in style.', short: 'Oversized utility denim jacket for layering', images: PRODUCT_IMAGES.jacket,
      variants: SIZES.map((s, i) => ({ size: s, color: 'Blue', colorCode: '#4B9CD3', price: 3499, mrp: 5499, stock: 20, images: PRODUCT_IMAGES.jacket, sku: generateSKU('HM5', i + 1) })),
      tags: ['hm', 'denim', 'jacket', 'winter', 'utility'], isFeatured: false, isNewArrival: false, isTrending: true, isBestSeller: false },
    { name: 'Fossil Heritage Automatic Watch', brand: 'fossil', category: 'watches', vendor: 3,
      desc: 'The Fossil Heritage Automatic features an exhibition caseback showcasing the intricate self-winding mechanical movement. Crafted with a premium stainless steel bracelet.', short: 'Heritage automatic watch with exhibition caseback', images: PRODUCT_IMAGES.watch,
      variants: [{ size: 'Free Size', color: 'Silver', colorCode: '#C0C0C0', price: 21999, mrp: 27999, stock: 5, images: PRODUCT_IMAGES.watch, sku: generateSKU('FOS3', 1) }],
      tags: ['fossil', 'watch', 'automatic', 'heritage', 'luxury'], isFeatured: true, isNewArrival: true, isTrending: false, isBestSeller: true },
    { name: 'Wildcraft Adventurer Duffel Backpack', brand: 'wildcraft', category: 'bags', vendor: 3,
      desc: 'A hybrid travel bag that converts from a carry-on duffel to an active backpack. Offers water-resistant shell, heavy-duty straps, and a separate shoe compartment.', short: 'Convertible hybrid duffel backpack for travelers', images: PRODUCT_IMAGES.bag,
      variants: [{ size: 'Free Size', color: 'Black', colorCode: '#000000', price: 3999, mrp: 5999, stock: 15, images: PRODUCT_IMAGES.bag, sku: generateSKU('WLD4', 1) }],
      tags: ['wildcraft', 'duffel', 'backpack', 'travel', 'convertible'], isFeatured: false, isNewArrival: false, isTrending: false, isBestSeller: false },
    { name: 'US Polo Assn. Men Cable-Knit Sweater', brand: 'us-polo', category: 'men', vendor: 0,
      desc: 'Keep warm in preppy style with this classic cable-knit sweater. Made from ultra-soft cotton yarn, featuring the signature double-horseman embroidery on the chest.', short: 'Classic cable-knit sweater in ultra-soft cotton', images: PRODUCT_IMAGES.hoodie,
      variants: SIZES.map((s, i) => ({ size: s, color: 'Navy Blue', colorCode: '#001F5B', price: 2799, mrp: 3999, stock: 18, images: PRODUCT_IMAGES.hoodie, sku: generateSKU('USP3', i + 1) })),
      tags: ['us polo', 'sweater', 'cable knit', 'men', 'classic'], isFeatured: false, isNewArrival: false, isTrending: false, isBestSeller: true },
    { name: 'ONLY High-Rise Pleated Tennis Skirt', brand: 'only', category: 'women', vendor: 0,
      desc: 'A preppy, active high-rise pleated skirt from ONLY. Built-in inner shorts for comfort and mobility during play or casual hangouts. Soft-structured design.', short: 'Preppy pleated tennis skirt with inner shorts', images: PRODUCT_IMAGES.dress,
      variants: ['XS', 'S', 'M', 'L'].map((s, i) => ({ size: s, color: 'White', colorCode: '#FFFFFF', price: 1599, mrp: 2499, stock: 20, images: PRODUCT_IMAGES.dress, sku: generateSKU('ONL3', i + 1) })),
      tags: ['only', 'skirt', 'pleated', 'tennis', 'women'], isFeatured: false, isNewArrival: true, isTrending: false, isBestSeller: false },
    { name: 'BIBA Embroidered Silk Lehenga Choli', brand: 'biba', category: 'ethnic-wear', vendor: 1,
      desc: 'Make a royal statement in this BIBA Lehenga Choli. Beautifully embroidered art silk lehenga paired with a matching choli and sheer net dupatta with gold lace borders.', short: 'Royal embroidered silk lehenga choli set', images: PRODUCT_IMAGES.saree,
      variants: [{ size: 'Free Size', color: 'Maroon', colorCode: '#800000', price: 14999, mrp: 19999, stock: 4, images: PRODUCT_IMAGES.saree, sku: generateSKU('BIB4', 1) }],
      tags: ['biba', 'lehenga', 'ethnic', 'silk', 'embroidery', 'festive'], isFeatured: true, isNewArrival: true, isTrending: true, isBestSeller: false },
    { name: 'Adidas Ultraboost Kids Active Shoes', brand: 'adidas', category: 'kids', vendor: 4,
      desc: 'Provide supreme comfort to growing active feet with these Kids Ultraboost running shoes. Features elastic laces, breathable mesh, and the iconic Boost cushioning.', short: 'Ultraboost cushioning designed for kids', images: PRODUCT_IMAGES.shoes,
      variants: ['4-5Y', '6-7Y', '8-9Y', '10-11Y'].map((s, i) => ({ size: s, color: 'Blue', colorCode: '#1e90ff', price: 4499, mrp: 5999, stock: 15, images: PRODUCT_IMAGES.shoes, sku: generateSKU('ADIK', i + 1) })),
      tags: ['adidas', 'kids', 'shoes', 'ultraboost', 'sport'], isFeatured: false, isNewArrival: true, isTrending: false, isBestSeller: false },
    { name: 'Puma Classic Suede Retro Sneakers', brand: 'puma', category: 'footwear', vendor: 2,
      desc: 'The original Puma Suede classic has been a streetwear icon since 1968. Features rich genuine suede leather upper, gold foil branding, and a durable rubber outsole.', short: 'Iconic 1968 retro suede sneakers in navy', images: PRODUCT_IMAGES.shoes,
      variants: SHOE_SIZES.map((s, i) => ({ size: s, color: 'Navy Blue', colorCode: '#001F5B', price: 4999, mrp: 6999, stock: 18, images: PRODUCT_IMAGES.shoes, sku: generateSKU('PUM3', i + 1) })),
      tags: ['puma', 'suede', 'sneakers', 'retro', 'streetwear'], isFeatured: true, isNewArrival: false, isTrending: true, isBestSeller: true }
  ];
  ...
  console.log('❓ Creating product Q&As...');
  const qaTexts = [
    {
      question: 'Is this product true to size or should I size up?',
      answers: [
        { answer: 'It fits true to size. I ordered my regular size M and it fits perfectly.', isVendor: false },
        { answer: 'We recommend ordering your standard size, but if you prefer a relaxed fit, please size up.', isVendor: true }
      ]
    },
    {
      question: 'What is the material composition of this item?',
      answers: [
        { answer: 'It is made of a premium cotton blend. Very soft and breathable!', isVendor: false }
      ]
    },
    {
      question: 'Does the color fade after multiple washes?',
      answers: [
        { answer: 'I have washed it 3 times now, and there is no color fading at all. Still looks vibrant.', isVendor: false },
        { answer: 'We use high-quality dyes to prevent fading. Wash inside out in cold water for best results.', isVendor: true }
      ]
    },
    {
      question: 'Is this suitable for warm summer weather?',
      answers: [
        { answer: 'Yes, it is very lightweight and perfect for summers.', isVendor: false }
      ]
    }
  ];
  ...
  for (let i = 0; i < Math.min(50, createdProducts.length); i++) {
    const product = createdProducts[i];
    const numQAs = randomInt(1, 2);
    for (let j = 0; j < numQAs; j++) {
      const customer = randomFrom(customers);
      const qa = randomFrom(qaTexts);
      
      const seededAnswers = qa.answers.map((ans) => {
        const answeringUser = ans.isVendor ? randomFrom(vendorUsers) : randomFrom(customers);
        const vendorDoc = ans.isVendor ? vendorStores.find((v) => String(v.user) === String(answeringUser._id)) : undefined;
        return {
          user: answeringUser._id,
          vendor: vendorDoc?._id,
          isVendor: ans.isVendor,
          answer: ans.answer,
      ...
      await Question.create({
        ...
        isAnswered: seededAnswers.length > 0,
        createdAt: new Date(Date.now() - randomInt(10, 20) * 24 * 60 * 60 * 1000)
      });
    }
  }
  ...
  await Banner.create([
    {
      title: 'VOGUE EDIT: MONOCHROME 2026',
      subtitle: 'Premium minimalist coordinates and luxury essential layers. Elevate your daily rotation.',
    ...
    {
      title: 'UP TO 50% OFF: SNEAKER CULTURE',
      subtitle: 'Limited-edition drops from Nike, Adidas & Puma. Unleash your active performance lifestyle.',
      image: 'https://images.unsplash.com/photo-1552346154-21d32810aba3?auto=format&fit=crop&w=1600&q=80',
      mobileImage: 'https://images.unsplash.com/photo-1552346154-21d32810aba3?auto=format&fit=crop&w=800&q=80',
    ...
    {
      title: 'ETHNIC OPULENCE',
      subtitle: 'Exquisite silk lehengas and hand-embroidered heritage sherwanis for every festive occasion.',
      image: 'https://images.unsplash.com/photo-1583391733981-8498408ee4b2?auto=format&fit=crop&w=1600&q=80',
      mobileImage: 'https://images.unsplash.com/photo-1583391733981-8498408ee4b2?auto=format&fit=crop&w=800&q=80',
    ...
    {
      title: 'ACCESSORIES: THE FINE PRINT',
      subtitle: 'Luxury watches, leather handbags, and high-fashion statement sunglasses.',
      image: 'https://images.unsplash.com/photo-1524498250077-3a819b76e246?auto=format&fit=crop&w=1600&q=80',
      mobileImage: 'https://images.unsplash.com/photo-1524498250077-3a819b76e246?auto=format&fit=crop&w=800&q=80',
